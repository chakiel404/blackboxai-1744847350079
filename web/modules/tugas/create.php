...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $db->prepare("
                INSERT INTO tugas (judul, deskripsi, file_path, kelas_id, 
                                 mata_pelajaran_id, guru_id, tenggat_waktu, dibuat_pada) 
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
            ");
            $stmt->bind_param("sssiiss", $judul, $deskripsi, $file_path, 
                            $kelas_id, $mata_pelajaran_id, $selected_guru_id, $tenggat_waktu);
            $stmt->execute();

            redirect('/web/modules/tugas/index.php', 'Assignment created successfully.', 'success');
        } catch (Exception $e) {
                    ...
                    <form method="POST" action="" enctype="multipart/form-data" class="needs-validation" novalidate>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" class="form-control" id="tenggat_waktu" name="tenggat_waktu" 
                                   value="<?php echo isset($_POST['tenggat_waktu']) ? htmlspecialchars($_POST['tenggat_waktu']) : ''; ?>" 
                                   required>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('tenggat_waktu').min = tomorrow.toISOString().slice(0, 16);
});
